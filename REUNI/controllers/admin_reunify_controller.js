...
router.get("/admin_reunify", function(req, res) {
  ...
  // Find Checked in Parent and all students where the parent government ID matches 
  // in both tables (join on foreign key) and the student status is not equal to 
  // 'Released' and then order the list by parent_govt_id -- to get the list in 
  // student - parent order, as the parent that checked in can have more than one student to pick up.
  ...
  .then(function(dbReunify) {
    ...
    console.log("admin_reunify_find: ", dbReunify);
